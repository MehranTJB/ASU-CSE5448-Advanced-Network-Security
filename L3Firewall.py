...
class Firewall (EventMixin):
    ...
    # source MAC address is found in the Tlogs dictionary, this traffic needs to be checked for spoofed source IP address
    ...
    #          Append the new rule into the CSV file and resend Firewall rules to the OVS switch.
    #      else
    #          this traffic is considered ligitimate and adds source MAC/IP addresses to the Tlogs dictionary for future investigations
       '''
    ...
    def UpdateCSV(self, srcmac='any', srcip='any', dstip='any'):
        ...
        if AllowAdd:       # Current rule is a new one and I add this rule to the firewall.
            ...
            # new rule, hence I want to mitigate DDoS attack againt target host, then I specify detination IP address
    ...
    # In the first part I want to detect Spoofed source IP address, In orderto do this I lookup for source IP address
    # in the Tlogs dictionary that I used to hold spoofed MAC/IP addresses. If I don't find this IP address in the
    # Tlogs dictionary, then I need to check source MAC addreses, IF I find another record with the same source MAC address
    ...
    def TrafficAnalyser(self, packet, match=None, event=None):
        ...
        if packet.type == packet.IP_TYPE:
            ...
            if packet.src in self.Tlogs:
                ...
                # addrersses in the Tlogs dictionary. If I find the similar record with the same source IP and MAc address, based
                # on the lab's asumptions I conclude the traffic is ligitimate and I print out a message at controller's console
                # "Duplicate Entry".
                if self.Tlog.get(packet.src) == [ip_packet.srcip, ip_packet.dstip, event.port]:
                    ...
                    if newip != ip_packet.srcip:
                        ...
                        # source IP addresses to to protect victim from DoS attack.
                        srcmac = str(packet.src)
                        srcip = None
                        dstip = str(ip_packet.dstip)
                        # I call the UpdateCSV function to add new rules based on the spoofed MAC or IP addresse detection.
            ...
            # then we need to recall installFlowcfunction to resend latest filrewall's rules from controller to the OVS switch.
    ...
    # I modified this function to call TrafficAnalyser function to find suspicious traffic (DoS traffic with Spoofed-
    # source MAC or IP addresse) by examining the source MAC and IP addresses.
